...
.factory('ManageApi', ['$resource','apiLinks', function($resource, apiLinks) {
  ...
  ManageApi.prototype.manage_users = function(){
    return this.service.manage_users();
  };
  ManageApi.prototype.delete_user = function(user_id){
    return this.service.delete_user({id: user_id});
  };
  ManageApi.prototype.disassociate = function(user_id){
    return this.service.disassociate({id: user_id});
  };
  ManageApi.prototype.edit_user = function(user){
    var user_json = {first_name: user.first_name, last_name: user.last_name, email: user.email, phone_no: user.phone_no, id: user.id};
  ...
  ManageApi.prototype.add_user = function(user){
    var user_json = {first_name: user.first_name, last_name: user.last_name, username: user.username,
      password: user.password, email: user.email, phone_no: user.phone_no, role: user.role, parent_id: user.parent_id};
    if(user.role == 4){
      user_json.region_id = user.region_id;
    }
